...
**Run logs** (`internal/cli/runlog.go`): every run, exec and task tees raw output to `~/.rr/logs/<name>-<timestamp>/output.log` via `logs.OpenRunLog`, using the same directory layout and retention (`logs:` in the global config) as parallel runs. After the command finishes, `attachRunOutcome` reads back the tail of that log and calls `formatters.ParseRunOutcome` once to fill `details.summary`, `details.failures` and `details.no_tests`. `--tail N` reprints the last N log lines after the result. `rr logs` lists these directories and `rr logs clean` applies retention.
...
2. **Provenance check**: the remote root holds a `.rr-source` marker (`internal/sync/marker.go`) with the source path, hostname, machine ID, branch, HEAD and worktree of the last sync. The machine ID (a hash of the hardware UUID on macOS, `/etc/machine-id` on Linux) identifies the machine, since macOS renames the hostname per network; older markers without one fall back to the hostname. If it names a different tree or machine, sync emits a `source_mismatch` warning before overwriting. rsync is told to protect the marker so `--delete` never removes it.
