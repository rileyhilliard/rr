...
- **Parallel subtasks get the same env and setup as single tasks** - Env merges host `env`, then `defaults.env`, then the task's `env` (later wins), and host `setup_commands` plus `defaults.setup` run after the `cd` into the project directory. Subtasks used to get only the task's `env` and the host's setup commands, run before the `cd`. Env values go through one command builder for single and parallel runs, local and remote: values are double-quoted, so `$HOME` and `$PATH` expand the same way everywhere (`PATH: "$HOME/.local/bin:$PATH"` used to arrive literally in remote parallel subtasks), backticks and double quotes are literal, and exports are sorted by name.
...
- **`rr sync --dry-run` changes nothing on the remote** - Lockfile invalidation ran before rsync regardless of `--dry-run`, and dry runs skip the lock, so a dry run could delete remote `node_modules`/`.venv` out from under another run. Dry runs now list what would be invalidated, and skip the provenance marker and worktree pruning.
- **Env var names are validated** - Host `env`, `defaults.env`, and task `env` keys must be valid shell variable names (letters, digits, underscores, not starting with a digit). A name like `app.name` or `X;rm` now fails validation with `CONFIG_INVALID` naming the key. Keys are inserted unquoted into the remote `export`, so an invalid one either broke the command or changed what it ran.
