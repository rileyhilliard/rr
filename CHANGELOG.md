...
- **A task with `depends` printed its stage display to stdout in structured mode** - The `━━━ Stage 1/2 ━━━` headers and completion lines went to stdout mixed into the tasks' output. They now print only with `--pretty`, and structured stdout carries only the tasks' own output.
