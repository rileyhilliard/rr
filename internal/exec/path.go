...
func GenerateSetupSuggestion(result *PathProbeResult, hostName string) string {
	...
	sb.WriteString("  - The tool is installed but not in any standard location\n\n")
	sb.WriteString("Fixes:\n\n")
	sb.WriteString(fmt.Sprintf("  1. Install '%s' on the remote machine\n\n", result.Command))
	sb.WriteString("  2. If already installed, find it and add to setup_commands:\n")
	sb.WriteString(fmt.Sprintf("     ssh %s \"which %s\"  # if it works in interactive shell\n", hostName, result.Command))
	sb.WriteString(fmt.Sprintf("     ssh %s \"command -v %s || find /opt /usr/local $HOME -name %s -type f 2>/dev/null | head -5\"\n\n", hostName, result.Command, result.Command))
	sb.WriteString("  3. Add the directory to PATH via setup_commands:\n")
	sb.WriteString("     hosts:\n")
	sb.WriteString(fmt.Sprintf("       %s:\n", hostName))
	sb.WriteString("         setup_commands:\n")
	sb.WriteString("           - export PATH=/opt/homebrew/bin:$HOME/.local/bin:$PATH\n")
